...
__addNamespace(this, {"cms":{"ui":{"tag":{"attributes":{}},"attributes":{},"tags":{"form":{},"interface":{},"media":{},"visualizer":{}}},"core":{},"helper":{}},"slikland":{"crypt":{},"mara":{}}});
var __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };
...
if (!EventDispatcher) {
  EventDispatcher = (function() {
     ...
     */
    EventDispatcher.prototype.on = function(p_event, p_handler, useCapture) {
      if (useCapture == null) {
        useCapture = false;
      }
      if (!this._events) {
      ...
      if (!(__indexOf.call(this._events[p_event], p_handler) >= 0)) {
        if (useCapture) {
          return this._events[p_event].push(p_handler);
        } else {
...
JSONUtils = (function() {
  ...
  JSONUtils.fromHTML = function(element) {
    var data;
    data = this._parseHTMLToJSON(element);
    console.log(data);
    return data;
  };
  JSONUtils._parseHTMLToJSON = function(element) {
    var arrayData, attr, child, childData, children, data, k, v, _i, _j, _len, _len1, _ref;
    ...
    for (_i = 0, _len = children.length; _i < _len; _i++) {
      child = children[_i];
      if (child.nodeType === 1) {
        childData = this._parseHTMLToJSON(child);
...
Node.prototype.getDOMTree = function() {
  var tree;
  tree = [this];
  if (this.parentNode) {
    tree = [].concat(this.parentNode.getDOMTree(), tree);
  }
  return tree;
};
Node.prototype.findClosest = function(query) {
  var closest, i, l, numClosest, target, targetTree, targets, tree, _i, _len;
  tree = this.getDOMTree();
...
BaseDOM = (function(_super) {
  ...
  BaseDOM.prototype.findClosest = function(query) {
    return this.element.findClosest(query);
  };
  BaseDOM.prototype.getDOMTree = function() {
    return this.element.findParents(getDOMTree);
  };
...
API = (function(_super) {
  ...
  function API(arg) {
    ...
    if (arg instanceof HTMLElement && arg.tagName.toLowerCase() === 'form') {
      this._form = arg;
      if (!(this._form.hasAttribute('autosubmit') && (this._form.getAttribute('autosubmit') === '0' || this._form.getAttribute('autosubmit') === 'false'))) {
  ...
  API.prototype.destroy = function() {
    return app.off('redraw', this._redraw);
  };
  API.get({
    url: function() {
      return this._url;
  ...
  API.get({
    loadingRatio: function() {
      return this._loadingRatio;
    }
  });
  API.set({
    loadingRatio: function(value) {
      return this._loadingRatio = Number(value);
    }
  });
  API.get({
    data: function() {
  ...
  API.prototype._addEventListeners = function() {
    var _ref;
    return (_ref = this._form) != null ? _ref.on('submit', this._submitForm) : void 0;
  };
  ...
  API.prototype.submit = function(data) {
    ...
    if (this._form) {
      ...
      if (this._form.hasAttribute('type') && this._form.getAttribute('type') === 'json') {
        ...
        data = JSON.stringify(this.constructor._parseJSON(this._form));
      } else {
    ...
    if (this._form && this._form.getAttribute('globalLoading')) {
      if (this._form.hasAttribute('globalLoading') && !isNaN(this._form.getAttribute('globalLoading'))) {
        this._loadingRatio = Number(this._form.getAttribute('globalLoading'));
      } else {
        this._loadingRatio = 0.7;
      }
  ...
  API.prototype.parseJSON = function(form) {
    return this.constructor.parseJSON(form);
  };
  API.parseJSON = function(form) {
    return this._parseJSON(form);
  };
  API._parseJSON = function(form) {
    var result;
    this._parsedElements = [];
    result = this._parseJSONElement(form);
    return result;
  };
  API._parseJSONElement = function(element) {
    var addC, data, ind, input, inputs, item, items, name, o, value, _i, _j, _len, _len1;
    ...
    for (_i = 0, _len = items.length; _i < _len; _i++) {
      item = items[_i];
      if (API._parsedElements.indexOf(item) >= 0) {
        continue;
      }
      API._parsedElements.push(item);
      name = item.getAttribute('json-name');
      if (data = API._parseJSONElement(item)) {
        if (!o[name]) {
          o[name] = [];
        }
        o[name].push(data);
    ...
    for (_j = 0, _len1 = inputs.length; _j < _len1; _j++) {
      input = inputs[_j];
      if (API._parsedElements.indexOf(input) >= 0) {
        continue;
      }
      API._parsedElements.push(input);
      value = input.value;
  ...
  API.prototype._loaded = function(e) {
    ...
    if (/__p[\d\.]+__/.test(t)) {
      ...
      this._triggerProgress(this._loadingRatio + p * (1 - this._loadingRatio));
    }
...
cms.ui.tag.attributes.Update = (function(_super) {
  ...
  Update.SELECTOR = ':not(modal)[update]';
  ...
  Plugin = (function(_super1) {
    ...
    Plugin.prototype._updateDelayed = function() {
      ...
      return this._updateTimeout = setTimeout(this._updateTargets, 250);
    };
...
cms.ui.attributes.Template = (function(_super) {
  ...
  Plugin = (function(_super1) {
    ...
    function Plugin(element) {
      ...
      this._template = this.attr('template');
      if (/^\>/.test(this._template)) {
        this._template = app.template.currentFile + this._template;
      }
...
cms.ui.attributes.Size = (function(_super) {
  ...
  Size.SELECTOR = '[height],[width],[min-width],[min-height]';
  function Size() {
  ...
  Size.prototype._update = function(data) {
    ...
    while (i-- > 0) {
      ...
      if (item.hasAttribute('min-height')) {
        ...
        _results.push(item.style['min-height'] = s);
      } else {
        _results.push(void 0);
      }
    }
...
cms.ui.tag.attributes.ShowModal = (function(_super) {
  ...
  Plugin = (function(_super1) {
    ...
    function Plugin(element) {
      ...
      if (/^\>/.test(this._modalRef)) {
        this._modalRef = app.template.currentFile + this._modalRef;
      }
      setTimeout(this._addEventListener, 1);
    }
    Plugin.prototype._addEventListener = function(e) {
      return this._element.on('click', this._click, true);
    };
    Plugin.prototype._click = function(e) {
      var data, target;
      data = {};
      if (this.attr('modalData')) {
        target = this.find(this.attr('modalData'));
        if (!target) {
          target = this.findClosest(this.attr('modalData'));
        }
        if (!target) {
          target = document.querySelector(this.attr('modalData'));
        }
        if (target) {
          data = JSONUtils.fromHTML(target);
        }
      }
      cms.ui.tags.Modal.show(this._modalRef, data);
      e.preventDefault();
...
cms.ui.tag.attributes.Service = (function(_super) {
  ...
  Plugin = (function(_super1) {
    ...
    function Plugin(element) {
      ...
      this._refreshInterval = Number(this.attr('serviceRefreshInterval'));
      this._element.on('update', this._update);
    ...
    Plugin.prototype._loadService = function() {
      ...
      if (this._element.hasAttribute('loadingRatio') && !isNaN(this._element.getAttribute('loadingRatio'))) {
        this._api.loadingRatio = Number(this._element.getAttribute('loadingRatio'));
      }
    ...
    Plugin.prototype._parseData = function() {
      var i, id, item, itemData, itemParams, items, multiple, name, paramSet, params, sort, sortValues, value, _i, _j, _len, _len1, _ref, _ref1;
      ...
      if (this.attr('serviceData')) {
        itemParams = {};
        items = document.querySelectorAll(this.attr('serviceData'));
...
var __hasProp = {}.hasOwnProperty;
cms.ui.tag.attributes.PushData = (function(_super) {
  var Plugin;
  __extends(PushData, _super);
  function PushData() {
    return PushData.__super__.constructor.apply(this, arguments);
  }
  PushData.SELECTOR = '[pushData]';
  PushData.prototype._update = function(data) {
  ...
  Plugin = (function(_super1) {
    ...
    function Plugin(element) {
      ...
      this._items = [];
      if ((_ref = element.itemData) != null ? _ref.items : void 0) {
        this._items = element.itemData.items;
      }
      this._element.on('update', this._update);
      this._element.on('remove', this._remove);
    }
    Plugin.prototype._update = function(e) {
      var data, pushData;
      pushData = this.attr('pushData');
      data = API.parseJSON(this.element);
      this._items.push(e.data);
      return app.template.renderBlock(this.element, {
        items: this._items
      });
    };
    Plugin.prototype._remove = function(e) {
      var i, _ref, _results;
      if (((_ref = e.data) != null ? _ref.target : void 0) != null) {
        i = this._items.length;
        _results = [];
        while (i-- > 0) {
          if (this._items[i] === e.data.target.itemData) {
            _results.push(this._items.splice(i, 1));
          } else {
            _results.push(void 0);
          }
        }
...
var __hasProp = {}.hasOwnProperty;
cms.ui.tag.attributes.PingService = (function(_super) {
  var Plugin;
  __extends(PingService, _super);
  function PingService() {
    return PingService.__super__.constructor.apply(this, arguments);
  }
  PingService.SELECTOR = '[pingservice]';
  PingService._queue = [];
  PingService.prototype._update = function(data) {
    var item, p, _i, _j, _len, _len1, _ref, _ref1, _results;
    _ref = data.add;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      item = _ref[_i];
  ...
  PingService.prototype.removeQueue = function(plugin) {};
  PingService.prototype.queueService = function(plugin, delay) {
    if (delay == null) {
      delay = 0;
    }
  };
  PingService.prototype._sortByOrder = function(a, b) {
    if (a.order > b.order) {
      return 1;
    }
    if (a.order < b.order) {
  ...
  Plugin = (function(_super1) {
    ...
    function Plugin(element) {
      ...
      this._refreshInterval = Number(this.attr('pingInterval'));
      if (isNaN(this._refreshInterval) || this._refreshInterval <= 0) {
    ...
    Plugin.prototype.destroy = function() {
      this._element.off('update', this._update);
      this._element.off('abort', this._abort);
      this._abort();
    ...
    Plugin.prototype._abort = function() {
      ...
      if ((_ref1 = this._api) != null) {
        _ref1.off(API.PROGRESS, this._onProgress);
      }
      return this._removeProgress();
    };
    Plugin.prototype._update = function(e, data) {
    ...
    Plugin.prototype._loadService = function() {
      ...
      this._isLoading = true;
      data = this._parseData();
      this._api = API.call(this._element.getAttribute('pingService'), data, this._serviceLoaded, this._serviceError);
      this._api.on(API.PROGRESS, this._onProgress);
      return this._showProgress();
    };
    Plugin.prototype._removeEventListeners = function() {
    ...
    Plugin.prototype._serviceLoaded = function(e, data) {
      var i, item, target, targets, _i, _len;
      i = data.length;
      while (i-- > 0) {
        item = data[i];
        if (item.target) {
...
cms.ui.tag.attributes.For = (function(_super) {
  ...
  Plugin = (function(_super1) {
    ...
    Plugin.prototype._updateTarget = function() {
    ...
    Plugin.prototype._updateDelayed = function() {
      clearTimeout(this._updateTimeout);
      return this._updateTimeout = setTimeout(this._updateTarget, 250);
    };
    Plugin.prototype._change = function(e) {
      return this._updateDelayed();
    };
...
slikland.crypt.Philo = (function() {
  function Philo() {}
  Philo.chars = function() {
    var cc, charCodes, ci, i, j, jl, l;
    if (!this._charMap) {
      ...
      while (++i < l) {
        ...
        jl = cc[1] - cc[0];
        while (++j <= jl) {
          this._charMap[ci++] = String.fromCharCode(j + cc[0]);
        }
      }
    }
    return this._charMap;
  };
  Philo.encode = function(value) {
    var b, bi, bin, bytes, c, chars, cl, i, isJson, l, res, si, sider, siderStr, skipper, sn, ts, vl;
    isJson = false;
    ...
    siderStr = this._lpad(sider.toString(2), 8);
    si = skipper;
    i = 0;
    l = bin.length;
    c = 0;
  ...
  Philo._lpad = function(value, size, char) {
    if (size == null) {
      size = 0;
    }
    if (char == null) {
  ...
  Philo._rpad = function(value, size, char) {
    if (size == null) {
      size = 0;
    }
    if (char == null) {
...
var __hasProp = {}.hasOwnProperty;
cms.ui.tag.attributes.Encode = (function(_super) {
  var Plugin;
  __extends(Encode, _super);
  function Encode() {
    return Encode.__super__.constructor.apply(this, arguments);
  }
  Encode.SELECTOR = '[encode]';
  Encode.prototype._update = function(data) {
    var item, p, _i, _j, _len, _len1, _ref, _ref1, _results;
    _ref = data.add;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      item = _ref[_i];
  ...
  Plugin = (function(_super1) {
    ...
    function Plugin(element) {
      ...
      this._form.on('submit', this._submit, true);
      this._element.setAttribute('rawname', this._element.name);
    }
    Plugin.prototype.destroy = function() {
      return Plugin.__super__.destroy.apply(this, arguments);
    };
    Plugin.prototype._submit = function() {
      return this.submit();
    };
    Plugin.prototype.submit = function() {
      this._cleanup();
      this._tempElement = this._element.cloneNode(true);
      this._element.name = '';
    ...
    Plugin.prototype._cleanup = function() {
      ...
      if (this._tempElement) {
        if ((_ref = this._tempElement.parentNode) != null) {
          _ref.removeChild(this._tempElement);
        }
        this._element.name = this._tempElement.name;
        return this._tempElement = null;
      }
    };
    return Plugin;
  })(BaseDOM);
...
cms.ui.tag.attributes.Draggable = (function(_super) {
  ...
  Plugin = (function(_super1) {
    ...
    function Plugin(element) {
      ...
      if (this.attr('draggableStyle')) {
        this._dragHandler.style = this.attr('draggableStyle');
      }
      this._dragHandler.on('mousedown', this._mouseDown);
      this._target = null;
      if (this.attr('draggableTarget')) {
        this._target = this.findClosest(this.attr('draggableTarget'));
      }
      if (!this._target) {
        this._target = this._element;
      }
    ...
    Plugin.prototype._mouseDown = function(e) {
      ...
      this._cloned = this._target.cloneNode(true);
      ...
      this._target.style.opacity = '0.6';
      this._target.parentNode.appendChild(this._cloned);
      this._moveCloned(e);
    ...
    Plugin.prototype._getDraggableChildren = function(contexts) {
      ...
      for (_i = 0, _len = contexts.length; _i < _len; _i++) {
        ...
        i = children.length;
        while (i-- > 0) {
          if (children[i] !== this._target && !children[i].hasAttribute('cloned')) {
            items.push({
              element: children[i]
            });
          }
    ...
    Plugin.prototype._switchElement = function(element, after) {
      ...
      if (targetIndex >= children.length - 1) {
        ...
        parent.insertBefore(this._target, children[targetIndex]);
      }
...
cms.ui.tag.attributes.Action = (function(_super) {
  ...
  Plugin = (function(_super1) {
    ...
    Plugin.prototype._click = function(e) {
      var action, evt, o, params, re, target, targets, _i, _len, _ref;
      action = this._element.getAttribute('action');
      if (/\[[^\[\]]+\]/.test(action)) {
        re = /\[([^\[\]]+)\]/g;
        while (o = re.exec(action)) {
          ...
          for (_i = 0, _len = targets.length; _i < _len; _i++) {
            ...
            if ((_ref = target.trigger) != null) {
              _ref.apply(target, [].concat(evt, params));
            }
          }
        }
      } else {
        this._api = API.call(action, null, this._apiComplete, this._apiError);
...
var __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };
cms.ui.tags.EvenHeight = (function(_super) {
  __extends(EvenHeight, _super);
  EvenHeight.SELECTOR = '.even-height';
  function EvenHeight() {
    this._resize = __bind(this._resize, this);
    this._update = __bind(this._update, this);
    EvenHeight.__super__.constructor.apply(this, arguments);
    window.addEventListener('resize', this._resize);
    app.on('redraw', this._update);
  }
  EvenHeight.prototype._update = function(data) {
    setTimeout(this._resize, 0);
    return setTimeout(this._resize, 500);
  };
  EvenHeight.prototype._resize = function() {
  ...
  Plugin = (function(_super1) {
    ...
    function Plugin(element) {
      ...
      if ((_ref = this._form) != null) {
        _ref.on('submit', this._submit, true);
      }
    }
    Plugin.prototype.destroy = function() {
      var _ref, _ref1, _ref2, _ref3;
      if ((_ref = this._input) != null) {
        _ref.off('blur', this._blur);
      }
      if ((_ref1 = this._input) != null) {
        _ref1.off('change', this._change);
      }
      if ((_ref2 = this._input) != null) {
        _ref2.off('input', this._change);
      }
      if ((_ref3 = this._form) != null) {
        _ref3.off('submit', this._submit, true);
      }
      return Plugin.__super__.destroy.apply(this, arguments);
    };
...
cms.ui.tags.form.Radio = (function(_super) {
  ...
  Radio.prototype._buildCheckbox = function(item) {
...
cms.ui.tags.form.Form = (function(_super) {
  ...
  Plugin = (function(_super1) {
    ...
    function Plugin(element) {
      ...
      app.on('redraw', this._redraw, true);
      this._redraw();
    }
    Plugin.prototype.destroy = function() {
      app.off('redraw', this._redraw);
      this._removeListeners();
      return Plugin.__super__.destroy.apply(this, arguments);
    };
    Plugin.prototype._addListeners = function() {
      this.attr('autosubmit', false);
      if (this._api == null) {
    ...
    Plugin.prototype._removeListeners = function() {
      var _ref, _ref1, _ref2, _ref3;
      if ((_ref = this._element) != null) {
        _ref.off('submit', this._submit);
      }
      if ((_ref1 = this._element) != null) {
        _ref1.off('change', this._change);
      }
      if ((_ref2 = this._element) != null) {
        _ref2.off('reset', this._reset);
      }
      return (_ref3 = this._element) != null ? _ref3.off('abort', this._abort) : void 0;
    };
    Plugin.prototype._redraw = function() {
      this._removeListeners();
      return setTimeout(this._addListeners, 10);
    };
    Plugin.prototype._abort = function() {
      var _ref;
    ...
    Plugin.prototype._apiComplete = function(e, data) {
      var evt, o, params, re, success, target, targets, _i, _len, _ref, _ref1, _ref2;
      this._element.reset();
      success = this.attr('success');
      if (success && success.length > 0) {
        if (/\[[^\[\]]+\]/.exec(success)) {
      ...
      if ((data != null ? (_ref1 = data.notification) != null ? (_ref2 = _ref1.message) != null ? _ref2.length : void 0 : void 0 : void 0) > 0) {
        if (!data.notification.type) {
          data.notification.type = 3;
        }
...
cms.ui.tags.form.Field = (function(_super) {
  ...
  Plugin = (function(_super1) {
    ...
    function Plugin(element) {
      ...
      var nameAttr, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9;
      Plugin.__super__.constructor.call(this, {
        element: element
      });
      ...
      if (nameAttr === 'date' || nameAttr === 'time') {
        ...
        if ((_ref = this._input) != null) {
          _ref.on('blur', this._blur);
        }
      } else {
        if ((_ref1 = (_ref2 = this._input) != null ? _ref2.tagName.toLowerCase() : void 0) === 'select' || _ref1 === 'textarea') {
          if ((_ref3 = this._input) != null) {
            _ref3.setAttribute('type', (_ref4 = this._input) != null ? _ref4.tagName.toLowerCase() : void 0);
          }
        }
        if ((_ref5 = this._input) != null ? _ref5.hasAttribute('type') : void 0) {
        ...
        if ((_ref6 = this._input) != null) {
          _ref6.on('focus', this._focus);
        }
        if ((_ref7 = this._input) != null) {
          _ref7.on('blur', this._blur);
        }
        if ((_ref8 = this._input) != null) {
          _ref8.on('change', this._change);
        }
        if ((_ref9 = this._input) != null) {
          _ref9.on('input', this._change);
        }
    ...
    Plugin.prototype._blur = function() {
      this._focused = false;
      this._checkFilled(this._isDateField);
      if (!this.find('validation')) {
        if (this.find('.validation-message')) {
          this.removeClass('invalid');
          return this.find('.validation-message').style['display'] = 'none';
        }
      }
    };
    Plugin.prototype._change = function() {
...
var __hasProp = {}.hasOwnProperty;
cms.ui.tags.form.Button = (function(_super) {
  __extends(Button, _super);
  function Button() {
    return Button.__super__.constructor.apply(this, arguments);
  }
  Button.SELECTOR = 'button';
  Button._IGNORE_ATTRIBUTES = ['action', 'href', 'duplicate', 'template', 'showModal'];
  Button.prototype._update = function(data) {
    var a, ignore, item, _i, _j, _len, _len1, _ref, _ref1, _results;
    _ref = data.add;
    _results = [];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      item = _ref[_i];
      ignore = false;
      _ref1 = this.constructor._IGNORE_ATTRIBUTES;
      for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
        a = _ref1[_j];
      ...
      if (ignore) {
        if (!item.hasAttribute('type')) {
          _results.push(item.setAttribute('type', 'button'));
        } else {
          _results.push(void 0);
        }
      } else {
...
var __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };
cms.helper.UploadHelper = (function(_super) {
  __extends(UploadHelper, _super);
  function UploadHelper() {
    ...
    this._remove = __bind(this._remove, this);
    var accept, el, k, v;
    UploadHelper.__super__.constructor.apply(this, arguments);
    this._loading = new cms.ui.Loading();
    this._loading.reset();
    this._loading.hide();
    this.appendChild(this._loading);
    accept = (this.attr('accept') || 'image/jpg;image/jpeg;image/png;image/gif').trim();
    this._apiPath = this.attr('api') || app.apiPath + 'upload';
    if (!accept || accept.length === 0) {
      accept = '*/*';
    }
  ...
  UploadHelper.set({
    apiPath: function(value) {
      return this._apiPath = value;
    }
  });
  UploadHelper.prototype._remove = function() {
    return this._setValue(null);
  };
  ...
  UploadHelper.prototype._startUpload = function() {
    if (!this._api) {
      this._apiStarter = new API(this._apiPath + '/start');
      this._apiStarter.reuse = true;
      this._apiStarter.type = 'json';
  ...
  UploadHelper.prototype._apiStarted = function(e, data) {
    this._loading.show();
    this._uploadId = data.id;
    this._apiUploadURL = this._apiPath + '/' + this._uploadId + '/';
    this._splitChunks();
  ...
  UploadHelper.prototype._splitChunks = function() {
    var b, end, i, init, l;
    this._chunks = [];
    i = 0;
    l = this._buffer.byteLength;
  ...
  UploadHelper.prototype._chunkProgress = function(e) {
    var p;
    p = 1 - ((this._chunks.length + 1) / this._numChunks) + e.progress / this._numChunks;
    return this._loading.progress = p;
  };
  UploadHelper.prototype._uploadNextChunk = function() {
    var chunk;
    if (this._chunks.length === 0) {
      this._completeUpload();
      return;
  ...
  UploadHelper.prototype._chunkUploadComplete = function() {
    return this._uploadNextChunk();
  };
  UploadHelper.prototype._chunkUploadError = function() {};
  UploadHelper.prototype._completeUpload = function() {
    this._loading.progress = 1;
    this._loading.hide();
    this._apiCompleter.url = this._apiPath + '/complete/' + this._uploadId;
    return this._apiCompleter.submit();
  };
  UploadHelper.prototype._apiCompleted = function(e, data) {
    this._uploading = false;
    this._setValue(data);
    return this.trigger('complete', data);
  };
  UploadHelper.prototype._setValue = function(value) {
    this.removeClass('deny');
    return this.removeClass('accept');
  };
  UploadHelper.prototype._parseFiles = function(files) {
    var file, i, totalSize;
    this._items = [];
    totalSize = 0;
    i = files.length;
  ...
  UploadHelper.prototype._loadNextFile = function() {
    var allLoaded, file, i;
    if (!this._fileReader) {
      this._fileReader = new FileReader();
      this._fileReader.addEventListener('load', this._fileLoaded);
  ...
  UploadHelper.prototype._fileLoaded = function() {
    this._currentItem['file'] = null;
    this._bufferView.set(new Uint8Array(this._fileReader.result), this._currentItem.init);
    delete this._currentItem['file'];
    return setTimeout(this._loadNextFile, 0);
  };
  UploadHelper.prototype._filesLoaded = function() {
    return this.upload();
  };
  UploadHelper.prototype.selectFile = function() {
    return this._click();
  };
  UploadHelper.prototype._click = function(e) {
    if (!this._fileBrowser) {
      this._fileBrowser = document.createElement('input');
      this._fileBrowser.setAttribute('type', 'file');
      if (this._acceptStr) {
  ...
  UploadHelper.prototype._fileSelect = function(e) {
    if (this._fileBrowser.files.length > 0) {
      return this._parseFiles(this._fileBrowser.files);
    }
  };
  UploadHelper.prototype._drop = function(e) {
    var accepts, files;
    e.preventDefault();
    files = e.dataTransfer.items;
    console.log(files);
  ...
  UploadHelper.prototype._dragOver = function(e) {
    e.preventDefault();
    return e.stopImmediatePropagation();
  };
  UploadHelper.prototype._dragEnd = function(e) {
    var items, _results;
    items = e.dataTransfer.items;
    if (items) {
      _results = [];
  ...
  UploadHelper.prototype._dragEnter = function(e) {
    var accepts, f, files;
    e.preventDefault();
    e.dataTransfer.dropEffect = 'move';
    f = e.dataTransfer.items[0].getAsFile();
  ...
  UploadHelper.prototype._dragLeave = function(e) {
    e.dataTransfer.dropEffect = 'none';
    this.removeClass('accept');
    return this.removeClass('deny');
  };
  UploadHelper.prototype._checkAccepts = function(files) {
    var accepts, i;
    accepts = true;
    i = files.length;
    if (i === 0) {
...
var __hasProp = {}.hasOwnProperty;
cms.ui.tags.media.Cropper = (function(_super) {
  var CropCanvas, Plugin, Size;
  __extends(Cropper, _super);
  function Cropper() {
    return Cropper.__super__.constructor.apply(this, arguments);
  }
  Cropper.SELECTOR = 'cropper';
  Cropper.prototype._update = function(data) {
    var i, inst, item, p_id, _i, _j, _len, _len1, _ref, _ref1, _results;
    if (!this._pluginInstances) {
      this._pluginInstances = [];
    }
  ...
  Plugin = (function(_super1) {
    ...
    Plugin._destroyPlugin = function(item) {};
    function Plugin(element) {
      this._sizeClick = __bind(this._sizeClick, this);
      this._parseSizes = __bind(this._parseSizes, this);
      this._canvasUpdate = __bind(this._canvasUpdate, this);
      this._resize = __bind(this._resize, this);
      this._update = __bind(this._update, this);
      this._uploadComplete = __bind(this._uploadComplete, this);
      Plugin.__super__.constructor.call(this, {
        element: element
      });
      this._uploadHelper = new cms.helper.UploadHelper(this);
      this._uploadHelper.on('complete', this._uploadComplete);
      this._uploadHelper.apiPath = this.attr('apiPath') || app.apiPath + 'image';
      this.addClass('cropper');
      this._canvas = new CropCanvas();
      this._canvas.on('update', this._canvasUpdate);
      this.appendChild(this._canvas);
      window.addEventListener('resize', this._resize);
      this._resize();
      this._parseSizes();
      if (this.attr('width')) {
        this._canvas.attr('width', this.attr('width'));
      }
      this.element.on('selectImage', this._uploadHelper.selectFile);
    }
    Plugin.get({
    ...
    Plugin.prototype.destroy = function() {
      var _ref;
      app.off('redraw', this._update);
      window.removeEventListener('resize', this._resize);
      if ((_ref = this._canvas) != null) {
        _ref.destroy();
      }
      return Plugin.__super__.destroy.apply(this, arguments);
    };
    Plugin.prototype._uploadComplete = function(e, data) {
      var currentData, size, _i, _len, _ref, _results;
      currentData = this._currentSize.data;
      currentData['src'] = data.path;
      this._canvas.setData(currentData, true);
      this._currentSize.updateSrc(currentData['src'], data['id']);
      this._keepScrollPos();
      if (this._currentSize["default"]) {
        _ref = this._sizes;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          size = _ref[_i];
          ...
          if (!size.data['src']) {
            _results.push(size.updateSrc(currentData['src']));
          } else {
            _results.push(void 0);
          }
        }
    ...
    Plugin.prototype._update = function() {
      return this._parseSizes();
    };
    Plugin.prototype._keepScrollPos = function() {
      var st;
      if (this._thumbs) {
        st = this._thumbs.element.scrollTop;
        return setTimeout((function(_this) {
          return function() {
            return _this._thumbs.element.scrollTop = st;
          };
        })(this), 0);
      }
    };
    Plugin.prototype.selectSize = function(index) {
      var i, item, st, _ref;
      this._keepScrollPos();
      _ref = this._sizes;
      for (i in _ref) {
      ...
      return this._canvas.setData(this._currentSize.data);
    };
    Plugin.prototype._resize = function() {
      ...
      return this._canvas.resize();
    };
    Plugin.prototype._canvasUpdate = function(e, data) {
      var size, _i, _len, _ref, _results;
      this._keepScrollPos();
      this._currentSize.updateCrop(data.crop, true);
      if (this._currentSize["default"]) {
        _ref = this._sizes;
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ...
          if (!size.cropSet) {
            _results.push(size.updateCrop(data.crop));
          } else {
            _results.push(void 0);
          }
        }
    ...
    Plugin.prototype._parseSizes = function() {
      var first, i, item, items, size, sizeData, _i, _j, _len, _len1, _ref, _results;
      this._sizes = [];
      items = this.findAll('size');
      if (items.length === 0) {
        app.on('redraw', this._update);
        return;
      }
      ...
      for (_i = 0, _len = items.length; _i < _len; _i++) {
        ...
        if (first) {
          ...
          sizeData = size.data;
          if (!sizeData['src']) {
            sizeData['src'] = this._defaultSize.data['src'];
          }
        }
      ...
      if (this._sizes.length > 0) {
        ...
        for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
          size = _ref[_j];
          size.on('click', this._sizeClick);
          this._thumbs.appendChild(size);
        }
        this.appendChild(this._thumbs);
        this.addClass('hasThumbs');
      }
      if (this.attr('height')) {
        if (this._thumbs) {
          this._thumbs.css({
        ...
        this._canvas.attr('height', this.attr('height'));
      }
      this._resize();
      i = this._sizes.length;
      _results = [];
      while (i-- > 0) {
        _results.push(this.selectSize(i));
      }
      return _results;
    };
    Plugin.prototype._sizeClick = function(e) {
      return this.selectSize(this._sizes.indexOf(e.currentTarget));
    };
    return Plugin;
  })(BaseDOM);
  CropCanvas = (function(_super1) {
    __extends(CropCanvas, _super1);
    function CropCanvas() {
      this._areaMouseUp = __bind(this._areaMouseUp, this);
      this._areaMouseMove = __bind(this._areaMouseMove, this);
      this._areaDown = __bind(this._areaDown, this);
      this._cornerMouseUp = __bind(this._cornerMouseUp, this);
      this._cornerMouseMove = __bind(this._cornerMouseMove, this);
      this._cornerDown = __bind(this._cornerDown, this);
      this._redrawArea = __bind(this._redrawArea, this);
      this.resize = __bind(this.resize, this);
      this._imageLoaded = __bind(this._imageLoaded, this);
      CropCanvas.__super__.constructor.call(this, {
        'element': 'div',
        'className': 'canvas'
      });
      this._buildCanvas();
    ...
    CropCanvas.prototype.destroy = function() {
      ...
      return CropCanvas.__super__.destroy.apply(this, arguments);
    };
    CropCanvas.prototype._buildCanvas = function() {
      var corner, k, _ref;
      this._container = new BaseDOM({
        'className': 'container'
      });
      this._preview = new BaseDOM({
        className: 'preview'
      });
      this._cropArea = new BaseDOM({
        element: 'div',
        className: 'area'
      });
      this._corners = {};
      ...
      this._corners['tl'] = corner;
      corner = new BaseDOM({
        element: 'div',
        className: 'corner'
      });
      this._corners['tr'] = corner;
      corner = new BaseDOM({
        element: 'div',
        className: 'corner'
      });
      this._corners['bl'] = corner;
      corner = new BaseDOM({
        element: 'div',
        className: 'corner'
      });
      this._corners['br'] = corner;
      _ref = this._corners;
      for (k in _ref) {
        corner = _ref[k];
        corner.name = k;
        corner.element.name = k;
        corner.element.on('mousedown', this._cornerDown);
        corner.addClass(k);
        this._cropArea.appendChild(corner);
      }
      this._cropArea.element.on('mousedown', this._areaDown);
      this._preview.appendChild(this._cropArea);
      ...
      return this._buildBG();
    };
    CropCanvas.prototype._buildBG = function() {
      var context;
      this._bgCanvas = document.createElement('canvas');
      this._bgCanvas.setAttribute('width', 10);
      this._bgCanvas.setAttribute('height', 10);
      context = this._bgCanvas.getContext('2d');
      ...
      return this.css({
        'background-image': 'url(' + this._bgCanvas.toDataURL() + ')'
      });
    };
    CropCanvas.prototype.setData = function(data, resetCrop) {
      if (resetCrop == null) {
        resetCrop = false;
      }
      if (!this._image) {
      ...
      this._cropRect = [].concat(data['crop']);
      this._cropWidth = data['width'];
      this._cropHeight = data['height'];
      this._aspectRatio = this._cropWidth / this._cropHeight;
      return this._container.appendChild(this._image);
    };
    CropCanvas.prototype._imageLoaded = function() {
      var s;
      ...
      s = this._imageWidth / this._imageHeight;
      this._imageAspect = s;
      this._preview.css({
        'display': ''
      });
    ...
    CropCanvas.prototype._triggerUpdate = function() {
      var data;
      data = {
        crop: this._cropRect,
        'src': this._image.src
    ...
    CropCanvas.prototype.resize = function() {
      var b, bounds, h, pBounds, s, w;
      if (!(this.attr('width') || this.attr('height'))) {
        ...
        if (this.element.parentNode) {
          pBounds = this.element.parentNode.getBoundingClientRect();
          if (pBounds.height > h) {
            h = pBounds.height;
          }
        }
      ...
      if (!w || !h) {
        this._resizeTimeout = setTimeout(this.resize, 1);
        return;
      }
      if (this._image) {
        s = this._imageWidth / this._imageHeight;
        if (w / h > s) {
          s = h / this._imageHeight;
        } else {
          s = w / this._imageWidth;
        }
        this._scaleWidth = this._imageWidth * s;
        this._scaleHeight = this._imageHeight * s;
        this._preview.element.style['width'] = this._image.style['width'] = this._scaleWidth + 'px';
        this._preview.element.style['height'] = this._image.style['height'] = this._scaleHeight + 'px';
        this._preview.element.style['left'] = this._image.style['left'] = (w - this._scaleWidth) * 0.5 + 'px';
        this._preview.element.style['top'] = this._image.style['top'] = (h - this._scaleHeight) * 0.5 + 'px';
      }
      return this._redrawArea();
    };
    CropCanvas.prototype._redrawArea = function() {
      var h, nh, nw, px, py, ratio, s, w;
      if (!this._cropRect) {
        return;
      }
      px = this._cropRect[0];
      ...
      nw = w;
      nh = h;
      ratio = this._aspectRatio / this._imageAspect;
      if (this._aspectRatio > s) {
        nw = h * ratio;
      } else {
        nh = w / ratio;
      }
      ...
      if (this._resetCrop) {
        s = 1;
        if (this._aspectRatio > this._imageAspect) {
          w = 1;
          h = ((this._imageWidth / this._cropWidth) * this._cropHeight) / this._imageHeight;
        } else {
          h = 1;
          w = ((this._imageHeight / this._cropHeight) * this._cropWidth) / this._imageWidth;
        }
        px = (1 - w) * 0.5;
        py = (1 - h) * 0.5;
      ...
      this._cropRect[3] = h;
      this._cropArea.css({
        'left': this._cropRect[0] * 100 + '%',
        'top': this._cropRect[1] * 100 + '%',
        'width': this._cropRect[2] * 100 + '%',
        'height': this._cropRect[3] * 100 + '%'
      });
      return this._triggerUpdate();
    };
    CropCanvas.prototype._removeEventListeners = function() {
      this._currentCorner = null;
      window.removeEventListener('mousemove', this._cornerMouseMove);
      window.removeEventListener('mouseup', this._cornerMouseUp);
      window.removeEventListener('mousemove', this._areaMouseMove);
      return window.removeEventListener('mouseup', this._areaMouseUp);
    };
    CropCanvas.prototype._addCornerEventListeners = function(corner) {
      this._currentCorner = corner;
      window.addEventListener('mousemove', this._cornerMouseMove);
      return window.addEventListener('mouseup', this._cornerMouseUp);
    };
    CropCanvas.prototype._addAreaEventListeners = function() {
      window.addEventListener('mousemove', this._areaMouseMove);
      return window.addEventListener('mouseup', this._areaMouseUp);
    };
    CropCanvas.prototype._cornerDown = function(e) {
      var target;
      target = e.currentTarget;
      e.preventDefault();
      e.stopImmediatePropagation();
      return this._addCornerEventListeners(target);
    };
    CropCanvas.prototype._cornerMouseMove = function(e) {
      var bh, bounds, bw, cornerName, corners, h, nh, nw, p, pos, px, py, ratio, s, w, _i, _len;
      cornerName = this._currentCorner.name;
      pos = MouseUtils.getMousePos(e);
      bounds = this._preview.getBounds();
      ...
      for (_i = 0, _len = corners.length; _i < _len; _i++) {
        ...
        switch (p) {
          case 't':
            py = pos[1] / bh;
            h = (this._cropRect[1] + this._cropRect[3]) - py;
            break;
          case 'b':
            h = pos[1] / bh - py;
            break;
          case 'l':
            px = pos[0] / bw;
            w = (this._cropRect[0] + this._cropRect[2]) - px;
            break;
          case 'r':
            w = pos[0] / bw - px;
        }
    ...
    CropCanvas.prototype._cornerMouseUp = function(e) {
      return this._removeEventListeners();
    };
    CropCanvas.prototype._areaDown = function(e) {
      var bh, bounds, bw, pos, target;
      pos = MouseUtils.getMousePos(e);
      this._initPos = pos;
      bounds = this._preview.getBounds();
      bw = bounds.right - bounds.left;
      bh = bounds.bottom - bounds.top;
      this._localPos = [pos[0] / bw - this._cropRect[0], pos[1] / bh - this._cropRect[1]];
      target = e.currentTarget;
      e.preventDefault();
      e.stopImmediatePropagation();
      return this._addAreaEventListeners();
    };
    CropCanvas.prototype._areaMouseMove = function(e) {
      var bh, bounds, bw, pos, px, py;
      pos = MouseUtils.getMousePos(e);
      bounds = this._preview.getBounds();
      bw = bounds.right - bounds.left;
      bh = bounds.bottom - bounds.top;
      px = (pos[0] / bw) - this._localPos[0];
      py = (pos[1] / bh) - this._localPos[1];
      if (px < 0) {
        px = 0;
      }
      if (px + this._cropRect[2] > 1) {
        px = 1 - this._cropRect[2];
      }
      if (py < 0) {
        py = 0;
      }
      if (py + this._cropRect[3] > 1) {
        py = 1 - this._cropRect[3];
      }
      this._cropRect[0] = px;
      this._cropRect[1] = py;
      return this._redrawArea();
    };
    CropCanvas.prototype._areaMouseUp = function(e) {
      return this._removeEventListeners();
    };
    return CropCanvas;
  })(BaseDOM);
  Size = (function(_super1) {
    __extends(Size, _super1);
    function Size(target) {
      this._imageLoaded = __bind(this._imageLoaded, this);
      this._loadThumb = __bind(this._loadThumb, this);
      this._click = __bind(this._click, this);
      ...
      this._data = {};
      this._default = false;
      this._parseData(target);
      this._buildInputs();
      this._label = new BaseDOM({
        'element': 'label'
      });
      this._label.html = this._data['name'] + ' (' + this._data['width'] + 'x' + this._data['height'] + ')';
      this.appendChild(this._label);
      this._container = new BaseDOM({
        'className': 'thumb-container'
      });
      this._thumbMask = new BaseDOM({
        'className': 'thumb-mask'
      });
      this._thumb = new BaseDOM({
        'className': 'thumb'
      });
      this._thumbMask.appendChild(this._thumb);
      this._container.appendChild(this._thumbMask);
      this.appendChild(this._container);
      this.element.on('click', this._click);
      setTimeout(this._loadThumb, 0);
    }
    Size.get({
      data: function() {
        return this._data;
    ...
    Size.set({
      selected: function(value) {
        ...
        if (value !== this._selected) {
          this._selected = value;
          return this.toggleClass('selected', this._selected);
        }
      }
    });
    Size.prototype._buildInputs = function() {
      this._input = document.createElement('data');
    ...
    Size.prototype._parseData = function(target) {
      var aspectRatio, crop, cropSet, h, k, name, optional, raw, src, v, value, w;
      ...
      if (target.hasAttribute('cropSet')) {
        ...
        if (value === 1 || (typeof value.toString === "function" ? value.toString().toLowerCase() : void 0) === 'true') {
          cropSet = true;
        } else {
          cropSet = false;
        }
      ...
      if (target.hasAttribute('optional')) {
        ...
        if (!(value === 0 || (typeof value.toString === "function" ? value.toString().toLowerCase() : void 0) === 'false')) {
      ...
      this._data['src'] = src;
      this._data['crop'] = crop;
      return this._data['cropSet'] = cropSet;
    };
    Size.prototype._click = function() {
      return this.trigger('click');
    };
    Size.prototype._loadThumb = function() {
      if (this._data['src']) {
        return this._load(this._data['src']);
      }
    };
    Size.prototype._load = function(src) {
      if (!this._image) {
        this._image = new Image();
        this._image.onload = this._imageLoaded;
        this._thumb.appendChild(this._image);
      }
      return this._image.src = src;
    };
    Size.prototype._imageLoaded = function() {
      this._image.style['width'] = '';
      this._image.style['height'] = '';
      this._imageWidth = this._image.naturalWidth || this._image.width;
      this._imageHeight = this._image.naturalHeight || this._image.height;
    ...
    Size.prototype.updateCrop = function(crop, direct) {
      if (direct == null) {
        direct = false;
      }
      if (direct) {
        if (this._data['crop'][0] !== crop[0] || this._data['crop'][1] !== crop[1] || this._data['crop'][2] !== crop[2] || this._data['crop'][3] !== crop[3]) {
          this._data['cropSet'] = true;
        }
      }
    ...
    Size.prototype._redraw = function() {
      var bounds, ch, cih, ciw, cix, ciy, cw, cx, cy, h, ih, iw, s, ss, w;
      bounds = this.getBounds();
      w = bounds.width;
      h = bounds.height;
      cx = this._data['crop'][0];
      cy = this._data['crop'][1];
      cw = this._data['crop'][2];
      ch = this._data['crop'][3];
      if (this._imageWidth && this._imageHeight) {
        iw = this._imageWidth;
        ih = this._imageHeight;
        cix = iw * cx;
        ciy = ih * cy;
        ciw = iw * cw;
        cih = ih * ch;
        ...
        cix = ciw * cx;
        ciy = cih * cy;
        this._image.style['left'] = -cix + 'px';
        this._image.style['top'] = -ciy + 'px';
        this._image.style['width'] = ciw + 'px';
        return this._image.style['height'] = cih + 'px';
      }
    };
    return Size;
  })(BaseDOM);
...
cms.ui.tags.Pagination = (function(_super) {
  ...
  Plugin = (function(_super1) {
    ...
    Plugin.prototype._update = function(e) {
      var data, i, index, l, numItems, page, total;
      data = e.data;
      if (data.pagination) {
        data = data.pagination;
      }
      total = data._total || data.total;
      numItems = data._numItems || data.numItems;
      index = data.iIndex || data.index;
      if (!data || !numItems) {
        return;
      }
      data.index = index;
...
var __hasProp = {}.hasOwnProperty;
cms.ui.tags.Modal = (function(_super) {
  var Plugin;
  ...
  Modal.show = function(reference, data) {
    if (data == null) {
      data = {};
    }
    if (reference) {
      return this._loadTemplate(reference, data);
    }
  };
  Modal._loadTemplate = function(ref, data) {
    var block;
    if (data == null) {
      data = {};
    }
    return block = app.template.templates.get(ref, (function(_this) {
      return function(block) {
        return _this._referenceLoaded(block, data);
      };
    })(this));
  };
  Modal._referenceLoaded = function(block, data) {
    var items;
    if (data == null) {
      data = {};
    }
    return items = block.render(data, document.body);
  };
  Modal.prototype._update = function(data) {
    var i, inst, item, p_id, _i, _j, _len, _len1, _ref, _ref1, _results;
    if (!this._pluginInstances) {
      this._pluginInstances = [];
    }
  ...
  Plugin = (function(_super1) {
    ...
    function Plugin() {
      ...
      this._commit = __bind(this._commit, this);
      this._showModal = __bind(this._showModal, this);
      this._mouseDown = __bind(this._mouseDown, this);
      Plugin.__super__.constructor.apply(this, arguments);
      this._element.on('showModal', this._showModal);
      this._element.on('commit', this._commit);
      this._element.on('cancel', this._cancel);
      this._element.on('mousedown', this._mouseDown);
    }
    Plugin.prototype.destroy = function() {
      this._element.off('showModal', this._showModal);
      this._element.off('commit', this._commit);
      this._element.off('cancel', this._cancel);
      this._element.off('mousedown', this._mouseDown);
      return Plugin.__super__.destroy.apply(this, arguments);
    };
    Plugin.prototype._mouseDown = function(e) {
      if (e.target === e.currentTarget && e.currentTarget === this._element) {
        if (!this.attr('close') || (this.attr('close') && (this.attr('close') === 1 || this.attr('close') === 'true'))) {
          return this._cancel();
        }
      }
    };
    Plugin.prototype._getData = function() {
      var data, form, forms, itemData, _i, _len;
      forms = this.findAll('form');
      data = {};
    ...
    Plugin.prototype._showModal = function(e) {
      var data, ref;
      data = this._getData();
      ref = e.data;
      if (/^\>/.test(ref)) {
    ...
    Plugin.prototype._commit = function(e) {
      var data, target, targets, _i, _j, _len, _len1;
      data = this._getData();
      console.log("COMMIT!", this.attr('update'));
      if (e.data) {
        try {
          data = ObjectUtils.merge(data, e.data);
        } catch (_error) {}
      }
      console.log(this.attr('commit'));
      if (this.attr('commit')) {
        targets = document.querySelectorAll(this.attr('commit'));
        console.log(targets);
        for (_i = 0, _len = targets.length; _i < _len; _i++) {
          target = targets[_i];
      ...
      if (this.attr('update')) {
        targets = document.querySelectorAll(this.attr('update'));
    ...
    Plugin.prototype._cancel = function() {
      return this.close();
    };
    ...
    if (ret) {
      ...
      for (_i = 0, _len = ret.length; _i < _len; _i++) {
        ...
        if ((this._children != null) && this._children.length > 0) {
          ...
          this._update(data, context);
        }
      }
    }
    return ret;
  };
  Block.prototype._update = function(data, context) {
    var _results;
    _results = [];
    for (k in data) {
      v = data[k];
      if (this._tag === 'tag') {
        if (k === 'html' || k === 'text' || k === 'content') {
          _results.push(context.innerHTML = v);
        } else {
          _results.push(context.setAttribute(k, v));
        }
      } else {
        _results.push(void 0);
      }
...
slikland.Mara = (function(_super) {
  ...
  Mara.get({
    currentFile: function() {
      var file, _ref;
      file = (_ref = this._renderData) != null ? _ref.file.replace(/\>.*?$/, '') : void 0;
      return file;
    }
  });
  Mara.get({
  ...
  Mara.prototype.render = function(file, data, context, callback, clearContext) {
    if (data == null) {
      data = {};
    }
    if (context == null) {
  ...
  Mara.prototype._templateLoaded = function(block) {
    ...
    if (this._renderData.context) {
      ...
      if (this._renderData.clearContext) {
  ...
  Mara.prototype._resetContext = function(target) {
    ...
    while (i-- > 0) {
      items[i] = children[i];
      try {
        if (!((_ref = items[i]) != null ? _ref.removable : void 0) === false) {
  ...
  Mara.prototype.renderBlockByReference = function(reference, data, reset, callback, context) {
    if (reset == null) {
      reset = false;
    }
    if (callback == null) {
    ...
    if (/^\>/.test(reference)) {
      reference = this._renderData.file + reference;
    }
    return this._templates.get(reference, this._referenceLoaded);
  };
  ...
  Mara.prototype.renderBlock = function(element, data, context, onlyChild) {
...
Main = (function() {
  ...
  Main.prototype._renderTemplate = function(e, data) {
    ...
    if (data.clearContext != null) {
      clearContext = data.clearContext;
    } else {
      clearContext = true;
    }
    return app.template.render(data.template, data.data || {}, target || app.templateContext, null, clearContext);
  };
  return Main;
})();
