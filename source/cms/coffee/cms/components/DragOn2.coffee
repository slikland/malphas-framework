class components.DragOn2 extends BaseDOM
	@SELECTOR: '.dragBox2'
	@ORDER: 0
	constructor:()->
		super
	...
	_create:()=>
		...
		leftDrag = $(@_element).attr("leftDrag")
		...
		@addinputLeftDrag = document.createElement("input")
		$(@addinputLeftDrag).attr('type', 'hidden')
		$(@addinputLeftDrag).attr('name', 'leftDrag')
		$(@addinputLeftDrag).attr('class', 'leftDrag')
		$(@addinputLeftDrag).attr('value', leftDrag)
		...
		$(@element).append(@addinputTopDrag,@addinputLeftDrag,@addinputHeightDrag,@addinputWidthDrag)
		dragOn2(topDrag,leftDrag)
