...
slikland.utils.keyboard.Shortcut = (function() {
  Shortcut.MODIFIERS = ['alt', 'ctrl', 'meta', 'shift', 'click'];
  ...
  Shortcut.prototype._mouseDown = function(e) {
    var item, items, k, key, mapped, modifiers, shortcut, _i, _j, _len, _len1, _ref, _results;
    key = '';
    mapped = [];
    modifiers = ['click'];
...
App = (function(_super) {
  ...
  App.project_version_raw = "SL_PROJECT_VERSION:1.1.2";
  App.project_date_raw = "SL_PROJECT_DATE:1480342374690";
...
LiveEditor = (function(_super) {
  ...
  LiveEditor.prototype._shortcutCommand = function(e, data) {
    if (!this._editting) {
      return;
    }
    if (this._shortcutCommands[data.shortcut]) {
      if (this._shortcutCommands[data.shortcut].command instanceof Function) {
        return this._shortcutCommands[data.shortcut].command(e, data);
      } else {
        return document.execCommand(this._shortcutCommands[data.shortcut].command, true, this._shortcutCommands[data.shortcut].value);
      }
    }
  };
  LiveEditor.prototype.execCommand = function(name, useUI, value) {
    return document.execCommand(name, useUI, value);
  };
...
Nav = (function(_super) {
  ...
  Nav.prototype._change = function() {
    var data, id, item, k, obj, pages, _ref;
    this._links = [];
...
Content = (function() {
  function Content() {
    this._linkClick = __bind(this._linkClick, this);
    this._parseLinks = __bind(this._parseLinks, this);
    this._change = __bind(this._change, this);
    this._contentLoaded = __bind(this._contentLoaded, this);
    this._onPaste = __bind(this._onPaste, this);
    this._linkCallback = __bind(this._linkCallback, this);
    ...
    this._contentEditor.addShortcutCommand('[cmd][alt]o', 'insertOrderedList');
    this._contentEditor.addShortcutCommand('[cmd][alt]l', 'insertUnorderedList');
    this._contentEditor.addShortcutCommand('[tab]', 'insertText', '\t');
    this._contentEditor.addShortcutCommand('[alt][click]', this._linkCallback);
    this._target.addEventListener('paste', this._onPaste);
  }
  Content.prototype._linkCallback = function(e, data) {
    var html, target;
    target = e.target;
    if (target.matches('a[href]')) {
      ...
      return this._contentEditor.execCommand('insertHTML', false, html);
    }
  };
  ...
  Content.prototype._change = function() {
    var content, data;
    data = {};
    content = this._target.innerHTML;
    content = content.replace(/\[\[(http.*?)\]\]/ig, '<a href="$1" target="_blank">$1</a>');
    data['content'] = this._target.innerHTML = content;
    data['page'] = this._currentPage;
    this._parseLinks();
    return API.call('php/editPage.php', data, null, null, 'json');
  };
  Content.prototype._parseLinks = function() {
    var a, anchors, nav, _i, _len, _results;
    anchors = this._target.querySelectorAll('a[id]');
    _results = [];
    for (_i = 0, _len = anchors.length; _i < _len; _i++) {
      a = anchors[_i];
      nav = document.querySelector('nav a[id="' + a.getAttribute('id') + '"]');
      if (nav) {
        a.removeEventListener('click', this._linkClick);
        a.addEventListener('click', this._linkClick);
        _results.push(a.setAttribute('href', nav.getAttribute('href')));
      } else {
        _results.push(void 0);
      }
    }
  ...
  Content.prototype._linkClick = function(e) {
    var target;
    target = e.currentTarget;
    if (!e.metaKey) {
      e.preventDefault();
